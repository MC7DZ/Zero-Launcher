...
public class CreateInstanceDialog extends JDialog {
    ...
    public static Optional<Instance> show(JFrame owner) {
    ...
    private CreateInstanceDialog(JFrame owner) {
        ...
        createBtn.addActionListener(e -> {
            ...
            result = new Instance(name, ver, lType, lVer);
            result.ramMb = ramSlider.getValue();
            result.imagePath = chosenImagePath[0];
            ...
            if (chosenModpackPath[0] != null) {
                ...
                result.modpackFilePath = chosenModpackPath[0];
                result.modpackInstallPath = path;
                result.useCustomDirectory = true;
                result.customDirectoryPath = path;
            } else {
                if (defaultDirRadio.isSelected()) {
                    result.useCustomDirectory = true;
                    result.customDirectoryPath = LauncherPaths.getDefaultMinecraftPath().toAbsolutePath().toString();
                } else if (customDirRadio.isSelected()) {
                    result.useCustomDirectory = true;
                    result.customDirectoryPath = customDirField.getText().trim();
                }
            }
            dispose();
        });
