...
function buildModCard(mod, directory, preservedIconHtml) {
  ...
  updateBtn.addEventListener('click', async (ev) => {
    ...
    let ok = false;
    try {
      await api.discoverDownload(directory, 'mod', info.file.url, info.file.filename, dlId);
...
async function installMissingDependencies(directory, instance, downloadId) {
  ...
  for (const mod of mods) {
    ...
    try {
      ...
      if (!res || !res.hits || res.hits.length === 0) continue;
      ...
      for (const dep of required) {
        ...
        await api.discoverDownload(directory, 'mod', primary.url, primary.filename, downloadId);
...
async function runUpdateAll(up, dlId) {
  ...
  try {
    for (let i = 0; i < up.length; i++) {
      ...
      try {
        await api.discoverDownload(directory, 'mod', u.file.url, u.file.filename, dlId);
...
function initApplyPresetOverlayEvents() {
  ...
  document.getElementById('btn-apply-preset-confirm').addEventListener('click', async () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      const r = selected[i];
      if (cancelled()) { wasCancelled = true; setPresetModStatus(r, 'skipped', 'Cancelled'); continue; }

      setPresetModStatus(r, 'downloading', 'Downloading…');
      progressLabel.textContent = `Applying preset… (${i + 1} / ${selected.length})`;
      if (dlWidgetGeneric) {
        dlWidgetGeneric.update(dlId, null, `${i} / ${selected.length} mods — ${r.name}`, (i / selected.length) * 100);
      }
      ...
      try {
        ...
        await api.discoverDownload(directory, 'mod', info.url, info.file_name, dlId);
        installedCount++;
...
async function downloadDiscoverSelection(hit, versionSelect, downloadBtn) {
  ...
  try {
    await api.discoverDownload(directory, hit.project_type, opt.dataset.fileUrl, opt.dataset.fileName, dlId);
