...
/// update and relaunch. The app exits as part of this — there's no return
/// to a "running" state on success.
#[tauri::command]
pub fn install_update(downloaded_path: String) -> Result<(), String> {
    ...
    {
        install_update_windows(&downloaded_path)
    }
    #[cfg(not(target_os = "windows"))]
    {
        install_update_linux(&downloaded_path)
    }
}
...
/// exit, moves the downloaded file over the current exe, relaunches it,
/// then deletes itself. We exit right after spawning it.
#[cfg(target_os = "windows")]
fn install_update_windows(downloaded_path: &std::path::Path) -> Result<(), String> {
...
/// swap the file directly with no helper script.
#[cfg(not(target_os = "windows"))]
fn install_update_linux(downloaded_path: &std::path::Path) -> Result<(), String> {
